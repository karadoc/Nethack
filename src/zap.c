...
struct obj *otmp;
{
	...
	case SPE_FORCE_BOLT:
		...
		} else if (u.uswallow || rnd(20) < 10 + AC_VALUE(find_mac(mtmp))) {
			dmg = d(2,12);
			if(dbldam) dmg *= 2;
...
boolean ordinary;
{
	...
	switch(obj->otyp) {
		...
		case WAN_LIGHTNING:
			...
			shieldeff(u.ux, u.uy);
			You("zap yourself%s.", Shock_immunity ?", but seem unharmed" :"");			
			if (Shock_immunity)
...
xchar sx, sy;
{
	...
	case ZT_FIRE:
	    if (Fire_resistance) {
		shieldeff(sx, sy);
		You("don't feel %shot.", Fire_immunity? "" : "very ");
	...
	case ZT_COLD:
	    if (Cold_resistance) {
		shieldeff(sx, sy);
		You("don't feel %scold.", Cold_immunity? "" : "very ");
	...
	case ZT_LIGHTNING:
	    if (Shock_resistance) {
		shieldeff(sx, sy);
		You("aren't %saffected.", Shock_immunity ?"" :"badly ");
...
int type;	/* either hero cast spell type or 0 */
{
	...
	return rn2(25+spell_bonus) > rn2(max(dex, 1));
	// With spell_bonus == 0
