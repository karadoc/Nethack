	...
	register struct obj	*otmp;
{
	...
	case POT_SICKNESS:
		...
		if (otmp->blessed) {
			...
			if(!Poison_immunity) {
			    if (otmp->fromsink)
					losehp((rnd(10)+5*!!(otmp->cursed))/(Poison_resistance? 2 : 1),
...
/*
** K-Mod, 1/apr/2011, karadoc
** dilution potions now are less effective at increasing max hp
**/
		/* original code
		healup(d(6 + 2 * bcsign(otmp), 8),
		       ...
		       !otmp->cursed, TRUE); */
		healup(d(6 + 2 * bcsign(otmp), 8),
		       (otmp->blessed ? 5 : !otmp->cursed ? 2 : 0)>>otmp->odiluted,
		       !otmp->cursed, TRUE);
// K-Mod end
	...
	case POT_FULL_HEALING:
		...
		healup(400, (4+4*bcsign(otmp))>>otmp->odiluted, !otmp->cursed, TRUE); // K-Mod
	...
	case POT_GAIN_ENERGY:			/* M. Stephenson */
		{	register int num;
			...
			u.uenmax += ((otmp->cursed) ? -num : num)>>otmp->odiluted;
	...
	case POT_OIL:				/* P. Winner */
		{
			...
			if (otmp->lamplit) {
				...
				losehp(d(Fire_immunity? 1 : (Fire_resistance? 2 : 4), 4),
				       "burning potion of oil", KILLED_BY_AN);
			    }
			} else if(otmp->cursed)
