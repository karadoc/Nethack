...
register xchar e_type;
{
	...
	// K-Mod: I disagree.
	// I don't think we need to reward this kind of cowardly action more than we already do.
...
** Increased the time taken to engrave.
** Things that use to be multi = -(len/10) are now -((len+2)/5)
**/
	switch(type){
	    default:
		multi = -((len+2)/5);
...
static const char *epitaphs[] = {
	...
	"Go away!",
	/* new messages, copied from sporkhack. ("I like them." - Karadoc) */
