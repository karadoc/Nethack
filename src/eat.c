...
/*
** K-Mod, 7/apr/2011, karadoc
** Eatting raw corpses is unhealthy. So it should abuse con.
** The abuse of con should match the 'tastes terrible' message
*/
	// food is vegan, but you like meat or dislike vegan food
	// or food is meat, but you like vegan food or dislike meat.
	if ((vegan(&mons[mnum]) && (carnivorous(youmonst.data) || !herbivorous(youmonst.data))) ||
		(!vegan(&mons[mnum]) && (herbivorous(youmonst.data) || !carnivorous(youmonst.data))))
		exercise(A_CON, FALSE);
// K-Mod end
	...
	if (mnum != PM_ACID_BLOB && !stoneable && rotted > 5L) {
		...
		{
			if (Poison_immunity || rn2(min(max(Luck, -1), 6) + 8) - 7 >= 0) // 0 .. 1/8 .. 1/2
