...
register struct attack  *mattk;
{
	...
	int dmg, armpro, permdmg, dmgreduct;
	char	 buf[BUFSZ];
...
/*
** K-Mod, 25/mar/2011, karadoc
** Rebalancing magic negation to make armpro=3 less powerful, and 2, 1 more powerful.
**/
	/* original code
	uncancelled = !mtmp->mcan && ((rn2(3) >= armpro) || !rn2(50)); */
	uncancelled = !mtmp->mcan && (!armpro || rn2(armpro*2)); // 0/1, 1/2, 3/4, 5/6
// K-Mod end
	...
	case AD_PHYS:
		if (mattk->aatyp == AT_HUGS && !sticks(youmonst.data)) {
			...
			if(mattk->aatyp == AT_WEAP && otmp) {
				...
				if (u.mh > 1 && u.mh > dmg-dmgreduct && // K-Mod
	...
	case AD_FIRE:
		...
		if (uncancelled) {
			...
			if (youmonst.data == &mons[PM_STRAW_GOLEM] ||
				...
				pline_The("fire doesn't feel %shot.", Fire_immunity? "" : "very ");
				dmg = Fire_immunity? 0 : (dmg+1)/2;
			}
	...
	case AD_COLD:
		...
		if (uncancelled) {
			...
			if (Cold_resistance) {
				pline_The("frost doesn't seem %scold.", Cold_immunity? "" : "very ");
				dmg = Cold_immunity? 0 : (dmg+1)/2;
			}
			if((int) mtmp->m_lev > rn2(20))
	...
	case AD_ELEC:
		...
		if (uncancelled) {
			You("get zapped!");
			if (Shock_resistance) {
				pline_The("zap doesn't %sshock you.", Shock_immunity? "" : "badly ");
				dmg = Shock_immunity? 0 : (dmg+1)/2;
			}
			if((int) mtmp->m_lev > rn2(20))
...
dopois:
	...
	case AD_DRIN:
		...
		mdamageu(mtmp, dmg);
		dmg = 0; // K-Mod, to avoid dealing damage again at the end.
...
do_stone:
			...
			if (!u.ustuck && !rn2(10)) {
				if (u_slip_free(mtmp, mattk)) {
					...
					pline("%s swings itself around you!",
						Monnam(mtmp));
					setustuck(mtmp);
					// K-Mod, just adding some more informaation...
					if (is_pool(mtmp->mx,mtmp->my) && !Swimming && !Amphibious)
						pline("You're being drowned!");
				}
			} else if(u.ustuck == mtmp) {
	...
	case AD_ACID:
		...
		hitmsg(mtmp, mattk);
		//if (!mtmp->mcan && !rn2(3))
		// K-Mod. Currently the only monsters with an active acid attack are the dragons, and I want it to work!
			if (Acid_resistance) {
	...
	*/
/*
** K-Mod, 8/apr/2011, karadoc
** Moved damage reduction calculation to the top. Some attacks set dmgreduct = 0;
*/
	/* original code
	if (dmg && u.uac < 0) {
	dmg -= rnd(-u.uac);
...
boolean ufound;
{
	...
	switch (mattk->adtyp) {
		...
		// don't reduce 'damage' for golem effects
		tmp = (Cold_resistance && !Cold_immunity) ? (tmp+1)/2 : tmp;
		goto common;
	    case AD_FIRE:
		not_affected |= Fire_immunity;
		tmp = (Fire_resistance && !Fire_immunity) ? (tmp+1)/2 : tmp;
		goto common;
	    case AD_ELEC:
		not_affected |= Shock_immunity;
		tmp = (Shock_resistance && !Shock_immunity) ? (tmp+1)/2 : tmp;
common:
