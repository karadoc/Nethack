...
register struct obj *obj;
{
	...
	if (Is_container(obj) || obj->otyp == STATUE) {
		 ...
		 *	   as cursed.
		 */
// K-Mod, 25/mar/2011, karadoc : bug fix. empty bag of holding should not weigh more
//		if (obj->otyp == BAG_OF_HOLDING)
		if (obj->otyp == BAG_OF_HOLDING && cwt > 0)
...
int x, y;
{
	...
	amount = (long)(1 + rnd(level_difficulty()+6) * rnd(20)); // lower after level 6 (roughly)
    if (gold) {
	gold->quan += amount;
    } else {
