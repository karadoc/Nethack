	...
	struct obj *bp;
{
	...
	case 5:
		...
		losestr(Poison_immunity? 1 : (Poison_resistance? rn1(2,1) : rn1(4,3)));
		//losehp(rnd(Poison_resistance ? 6 : 10),
		losehp(rnd(Poison_immunity? 3 : (Poison_resistance? 6 : 10)),
...
** made it possible to refresh spells sooner
*/
			/* original code
			} else if (spellknow(i) <= 1000) {*/
			} else if (spellknow(i) <= KEEN/4) {
// K-Mod end
...
/*
** K-Mod, 17/march/2011, karadoc
** Changed spell knowledge indicator. knowlegde > 25% " ", know > 25% > zero "!", zero >= know "*"
*/
			/* original code
			spellknow(i) ? " " : "*",*/
			(spellknow(i)>(KEEN/4))? " " : (spellknow(i)? "!" : "*"),
// K-Mod end
