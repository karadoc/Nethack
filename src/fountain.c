...
drinkfountain()
{
	...
	if (fate < 10) {
		...
		case 21: /* Poisonous */
			...
			{
				if (Poison_immunity || (rn2(min(max(Luck, -3), 3)+5) - 2 >= 0)) // between 0 and 3/4
				{
...
drinksink()
{
	...
	switch(rn2(20)) {
		...
		case 1: You("take a sip of very warm water.");
			break;
		case 2: You("take a sip of boiling hot water.");
			if (Fire_immunity)
				pline("It seems quite tasty.");
			else losehp(Fire_resistance? (rnd(6)+1)/2 : rnd(6), "sipping boiling water", KILLED_BY);
			break;
