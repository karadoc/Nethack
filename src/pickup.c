...
** show weighs flag (see later)
*/
	if (olist == invent || (olist->where == OBJ_CONTAINED && olist->ocontainer && carried(olist->ocontainer)))
		show_weight = TRUE;
	else
		show_weight = FALSE;
// K-Mod end
		    ...
		    */
			{
				...
				add_menu(win, obj_to_glyph(curr), &any,
					qflags & USE_INVLET ? curr->invlet : 0,
					def_oc_syms[(int)objects[curr->otyp].oc_class],
					ATR_NONE, tmpbuf, MENU_UNSELECTED);
			}
// K-Mod end
		}
		}
