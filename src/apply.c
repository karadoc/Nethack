...
** disabled attritube restoration for unicorn horn.
** changes marked bellow.
**/
#define ATTR_FIX 0	// K-Mod, max points to restore for each attribute (original default is 3)
#define PROP_COUNT 6		/* number of properties we're dealing with */
//#define ATTR_COUNT (A_MAX*3)	/* number of attribute points we might fix */
#define ATTR_COUNT (A_MAX*ATTR_FIX) // K-Mod
...
#define attr_trouble(Y) trouble_list[trouble_count++] = attr2trbl(Y)
	...
	for (idx = 0; idx < A_MAX; idx++) {
		...
		if (val_limit > ABASE(idx) + ATTR_FIX) val_limit = ABASE(idx) + ATTR_FIX; // K-Mod
