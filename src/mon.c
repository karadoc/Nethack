...
int  typ, fatal;
{
	...
	if(Poison_immunity || (Poison_resistance && rn2(min(max(Luck, -1), 6) + 8) - 7 >= 0)) // 0 .. 1/8 .. 1/2
	{
	...
	i = rn2(fatal + 20*thrown_weapon) + (Poison_resistance? 2 : 0);
	if(i == 0 && typ != A_CHA) {
		u.uhp = -1;
		pline_The("poison was deadly...");
	} else if(i <= 5) {
		/* Check that a stat change was made */
		if (adjattrib(typ, (thrown_weapon || Poison_resistance) ? -1 : -rn1(3,3), 1))
		    pline("You%s!", poiseff[typ]);
	} else {
