...
boolean disarm;
{
	...
	if (Luck > -13 && rn2(13+Luck) > 7) {	/* saved by luck */
		...
		case 6: {
			...
			if(Shock_resistance)  {
				You_feel("a surge of electricity.");
			    shieldeff(u.ux, u.uy);
				if (Shock_immunity)
				{
...
lava_effects()
{
	...
	{
		/* burn boots, not all armor; destroy_item doesn't work on
		armor anyway */
	...
	if (Wwalking) // check again, to see if the boots are still there!
	{
		...
		if (!Fire_immunity)
		{
			...
			pline_The("lava here burns you%s", Fire_resistance? "." : "!");
		}
	}
	else // no 'water' walking
	{
		if (!Fire_resistance)
		{
		...
		** I didn't know about the fire_damage formula; apparently the original author didn't either.
		...
		{
			obj2 = obj->nobj;
			if(is_organic(obj) && !obj->oerodeproof && obj->otyp != RIN_FIRE_RESISTANCE)
			{
				if(obj->owornmask)
				{
					Your("%s into flame!", aobjnam(obj, "burst"));
