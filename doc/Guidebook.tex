...
systems.
%.lp
\item[\ib{sparkle_speed}]
The speed of the sparkly effect when a monster (including yourself) is hit by an
attack to which it is resistant (default 1, zero = no sparkles).
%.lp
\item[\ib{dungeon}]
...
%.lp
\item[\ib{weight\_system}]
Specify the units used in inventory screen. The options are,
%.sd
%.si
{\tt metric} --- metric units
{\tt game} --- arbitrary game units
{\tt avoirdupois} --- American units
{\tt none} --- no weights displayed
%.ei
%.ed
(The default is 'metric'.)
%.lp
\item[\ib{sortloot}]
Controls the sorting of items in lists for
